...
def setupLogging():
   ...
   if(os.path.isdir(mediatorHome + "/log")):
      ...
      sys.stdout = open(logHome + "datachannel.out", "w", buffering=0)
      sys.stderr = open(logHome + "datachannel.err", "w", buffering=0)
      ...
      if 'restMediationServiceAuthentication' in datachannelProps:
         ...
         if(restMediationServiceAuthentication.lower() == "true"):
            ...
            if('restMediationServiceUsername' in datachannelProps and 'restMediationServicePassword' in datachannelProps):
               restMediationServiceUsername = datachannelProps['restMediationServiceUsername']
               restMediationServicePassword = datachannelProps['restMediationServicePassword']
               authHeader = 'Basic ' + base64.b64encode(restMediationServiceUsername + ":" + restMediationServicePassword)
               logging.debug("Auth header is: " + authHeader) 
            else:
